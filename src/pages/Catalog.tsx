          ...
          {filteredProducts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredProducts.map(product => {
                const isRecentlyAdded = addedIds.has(product.id);
                const quantityInCart = getItemQuantity(product.id);
                
                return (
                  <div key={product.id} className="bg-white border-2 border-black p-4 card-hover relative group flex flex-col">
                    <Link to={`/products/${product.id}`} className="block relative focus:outline-none focus:ring-4 focus:ring-[var(--color-wa-green)]">
                      ...
                      <div className="mb-4">
                        ...
                        <p className="text-xs font-bold text-slate-500 uppercase line-clamp-2">
                          {product.description}
                        </p>
                      </div>
                    </Link>
                    ...
                    <div className="mt-auto flex justify-between items-center border-t-2 border-black pt-4">
                      ...
                      <button 
                        ...
                        className={cn(
                          "font-bold p-2 px-4 uppercase text-[10px] tracking-widest border-2 transition-all",
                          isRecentlyAdded 
                            ? "bg-black text-white border-black" 
                            : "bg-[var(--color-wa-green)] text-white border-[var(--color-wa-green)] hover:bg-[#20bd5a]"
                        )}
                        title={isRecentlyAdded ? "Added to list" : "Add to list"}
                      >
                        {isRecentlyAdded ? (
                          <span className="flex items-center gap-1"><Check className="w-3 h-3"/> Added</span>
                        ) : (
                          <span className="flex items-center gap-1">+ Add <span className="hidden sm:inline">to List</span> {quantityInCart > 0 && `(${quantityInCart})`}</span>
                        )}
                      </button>
                    </div>
                  </div>
